...
class UserModel {
  static async create({ email, password, name, role = 'user' }) {
    ...
    const query = `
      ...
      VALUES (?, ?, ?, ?, NOW(), NOW())
    `;
    const [result] = await db.execute(query, [email, hashedPassword, name, role]);
    return { id: result.insertId, email, name, role };
  }

  static async findByEmail(email) {
    const query = 'SELECT id, email, password, name, role, created_at, updated_at FROM users WHERE email = ? LIMIT 1';
    const [rows] = await db.execute(query, [email]);
    return rows[0];
  }

  static async findById(id) {
    const query = 'SELECT id, email, name, role, created_at, updated_at FROM users WHERE id = ? LIMIT 1';
  ...
  static async updateRole(id, role) {
    const [res] = await db.execute('UPDATE users SET role = ?, updated_at = NOW() WHERE id = ?', [role, id]);
