...
class OrderModel {
  static async createOrder(connection, { userId, address, city, postalCode, country, totalAmount, paymentMethod, status = 'pending' }) {
    ...
    const [res] = await conn.execute(
      `INSERT INTO orders (user_id, address, city, postal_code, country, total_amount, payment_method, status, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`,
      [userId, address, city, postalCode, country, totalAmount, paymentMethod, status]
    );
    return res.insertId;
  }

  static async addOrderItem(connection, { orderId, productId, name, price, quantity }) {
    const conn = connection || db;
    const [res] = await conn.execute(
  ...
  static async getById(orderId) {
    const [rows] = await db.execute('SELECT * FROM orders WHERE id = ? LIMIT 1', [orderId]);
    return rows[0];
  }
  ...
  static async list({ page = 1, limit = 20, status }) {
    ...
    const [[{ count }]] = await db.query(`SELECT COUNT(*) as count FROM orders ${whereSql}`, params);
  ...
  static async listByUser(userId, { page = 1, limit = 20 }) {
    ...
    const [[{ count }]] = await db.query('SELECT COUNT(*) as count FROM orders WHERE user_id = ?', [userId]);
    return { rows, total: count };
  }
  ...
  static async statsOverview() {
    ...
    const [[{ total_revenue }]] = await db.query("SELECT IFNULL(SUM(total_amount),0) AS total_revenue FROM orders WHERE status IN ('paid','shipped','completed')");
