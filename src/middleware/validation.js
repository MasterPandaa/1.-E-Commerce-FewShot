...
exports.registerValidation = [
  ...
  body('password')
    .isLength({ min: 8 })
    .matches(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)/)
    .withMessage('Password minimal 8 karakter, harus mengandung huruf besar, kecil, dan angka'),
  body('name').trim().isLength({ min: 2, max: 50 }).withMessage('Nama harus 2-50 karakter')
];
...
exports.productUpdateValidation = [
  body('name').optional().trim().isLength({ min: 1 }).withMessage('Nama tidak boleh kosong'),
  ...
  body('category').optional().trim().isLength({ min: 1 }).withMessage('Kategori tidak boleh kosong'),
...
exports.checkoutValidation = [
  ...
  body('paymentMethod').isIn(['simulated_gateway']).withMessage('Invalid payment method')
];
